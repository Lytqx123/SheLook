#!/usr/bin/env bash
# ============================================================
# SheLook 一键部署脚本 — Linux/macOS
#
# 用法:
#   ./setup.sh                    # 全新部署
#   ./setup.sh --skip-build       # 跳过构建快速启动
#   ./setup.sh --clean --no-cache # 清理后无缓存重建
#   ./setup.sh --with-sdwebui     # 启用本地生图 (需 GPU)
#   ./setup.sh --stop             # 停止服务
#   ./setup.sh --restart          # 重启服务
#   ./setup.sh --logs backend     # 查看后端日志
#   ./setup.sh --status           # 查看状态
#   ./setup.sh --update           # 更新部署 (保留数据)
#   ./setup.sh --env staging      # 使用 staging 环境和覆盖配置
# ============================================================
